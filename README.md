...
| `prefix(names, prefix, goal)` | `names`: The names to be prefixed <br> ``: The prefix to apply <br> `goal`: The mappings that should be prefixed | Applies a prefix to all field and method names (but not class names) of mappings for some given names. |
