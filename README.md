...
| `diff(names, from, to, path)` | `names`: The names to create the diff on. <br> `from`: Old mappings <br> `to`: New mappings <br> `path`: Output file path | Writes a file which shows changes between two mappings. works best with unique names. |
...
| `mcpp(version)` | `version`: A MCP mappings version | Loads MCP patches for a given version from https://github.com/noeppi-noeppi/MappingUtilities/tree/master/mcp_patches. Required to merge MCP and official without conflicts. |
| `mcpu(version)` | `version`: A minecraft version with build identifier | Loads mcp unofficial mappings for a given version from https://github.com/noeppi-noeppi/MappingUtilities/tree/master/mcp_unofficial. |
