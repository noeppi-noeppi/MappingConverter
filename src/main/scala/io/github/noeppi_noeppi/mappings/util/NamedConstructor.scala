...
case class NamedConstructor(cls: ClassEntry, name: String, sig: MethodSignature, params: List[Option[String]], side: Side) {
  ...
  def forceKnown(newSide: Side): NamedConstructor = NamedConstructor(cls, name, sig, params, newSide)
}
